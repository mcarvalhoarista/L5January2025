...
logging.basicConfig(filename='ssd_log_execution.log', encoding='utf-8', level=logging.INFO)
...
for switch in devices_dict['switches']:
    ...
    try:
        ...
        try:
            ...
            logging.info (f"Switch {hostname} with model {model_id} have the following SSD:")
            ...
            try:
                ...
                logging.info (f"SSD_MODEL = {ssd_model}")
                logging.info (f"SSD_LIFETIME = {lifetime}")
                x += 1
            except:
            ...
            logging.info (f"Switch {hostname} with model {model_id} do NOT support the SSD command")
            y += 1
    except:
        logging.info (f"Switch {switch} do NOT have eAPI enabled or wrong credentials")
        y += 1
...
logging.info(f'Total of switches with SSD info= {x}')
logging.info(f'Total of switches without SSD info= {y}')
